...
PYBIND11_MODULE(RAISIMGYM_TORCH_ENV_NAME, m) {
  py::class_<VectorizedEnvironment<ENVIRONMENT>>(m, RSG_MAKE_STR(ENVIRONMENT_NAME))
      ...
      .def("reset2", &VectorizedEnvironment<ENVIRONMENT>::reset2)
      .def("observe", &VectorizedEnvironment<ENVIRONMENT>::observe)
      .def("observe2", &VectorizedEnvironment<ENVIRONMENT>::observe2)
      ...
      .def("setObOStatistics", &VectorizedEnvironment<ENVIRONMENT>::setObOStatistics)
      .def(py::pickle(
