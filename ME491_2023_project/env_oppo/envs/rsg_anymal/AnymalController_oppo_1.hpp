...
class AnymalController_20002001 {
 ...
 public:
  ...
  inline bool reset(raisim::World *world, double theta, double minObjSize = 0.15, int objIdx = -1, raisim::SingleBodyObject* objPtr = NULL) {
    ...
    // set target
    desiredR_ = 1.5 + uniDist_(gen_);
    desiredSpeed_ = targetSpeed_*sign(uniDist_(gen_));
    currentSpeed_ = 0.0;
    ...
    state << // my info(37)
             ...
             desiredR_, gc_.head(2).norm(), /// desiredR, currentR
             desiredSpeed_, currentSpeed_; /// desiredSpeed, currentSpeed
  ...
  void updateOnlyState(){
    ...
    Eigen::Vector3d vxy0;
    vxy0 << gv_[0], gv_[1], 0;
    currentSpeed_ = Eigen::Vector3d::UnitZ().cross(vxy0).dot(xy0);
  ...
  inline void recordReward(Reward *rewards) {
    ...
    double velReward = 0;
    // 1) magnitude
    velReward += 5*std::pow(currentSpeed_ - desiredSpeed_*std::min(timer_/200.0, 1.0), 2);
    // 2) rpy vel regularizer
    velReward += std::pow(bodyAngularVel_.norm(), 2);
    // 3) gv neet to perpendicular to gc
    velReward += 3*gc_.head(2).normalized().dot(gc_.head(2).normalized());

    rewards->record("a_forwardVel", velReward);


    RR_ << gc_.head(2).norm(), targetPos_.norm(), 0, 0;
    double rReward = (gc_.head(2).norm() - desiredR_);
    rewards->record("c_safety", rReward*rReward);
 ...
 private:
  ...
  double targetSpeed_;
  double desiredR_, desiredSpeed_, currentSpeed_;
  // history related
...
thread_local std::mt19937 raisim::AnymalController_20002001::gen_;
thread_local std::normal_distribution<double> raisim::AnymalController_20002001::normDist_(0., 1.);
thread_local std::uniform_real_distribution<double> raisim::AnymalController_20002001::uniDist_(0., 1.);
}