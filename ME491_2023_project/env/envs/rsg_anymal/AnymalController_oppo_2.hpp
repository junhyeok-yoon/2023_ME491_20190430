...
class AnymalController_20002000 {
 ...
 public:
  ...
  bool resetGivenPos(raisim::World *world, Eigen::Vector3d pos) {
    gc_init_.head(3) << pos;
  ...
  void setOppoState(Eigen::VectorXd& gcOppo, Eigen::VectorXd& gvOppo){
    gcOppo_ << gcOppo;
    gvOppo_ << gvOppo;
  }
  ...
  inline void recordReward(Reward *rewards) {
    ...
    rewards->record("a_forwardVel", 5*pow(targetSpeed_*std::min(timer_/200.0, 1.0) - bodyLinearVel_.head(2).norm(), 2) + bodyLinearVel_(2)*bodyLinearVel_(2) + gv_.segment(3,3).squaredNorm());
    ...
    if((gc_.head(2) - targetPos_).norm() < 0.35 || (gcOppo_.head(2) - targetPos_).norm() < 0.5){
      while((gc_.head(2) - targetPos_).norm() < 0.35 || (gcOppo_.head(2) - targetPos_).norm() < 2){
  ...
  inline const Eigen::VectorXd &getObservation() {
    ...
    state << // my info(37)
             ...
             targetPos_, /// opponent position
             (projectedRot*relativeTarget).head(2), /// opponent position relative to me
    obDouble_.head(95) << state, myHistory_[0], actionHistory_[0].cast<double>(), float(targetSpeed_);
    obDouble_.tail(88).setZero();
